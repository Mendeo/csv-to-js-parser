...
* order: порядок группировки. Задаётся только для тех столбцов, которые нужно группировать. Значения этого свойства показывают в каком порядке нужно группировать столбцы. Группировка идёт в порядке возрастания значения order. То есть сначала группируются столбцы с order: 1, потом внутри первой группировки группируются столбцы с order: 2 и т. д. Можно воспринимать свойство order аналогично конструкции Group By в языке SQL.
...
Требуется получить массив объектов JavaScript, в котором данные из таблицы 2 будут сгруппированы по полу. В этом случае description будет таким:
...
[
    {
        sex: 'male',
        age: [30, 30, 30, 31, 31, 31, 31, 31, 32, 32, 32],
        person_id: []
    },
    {
        sex: 'female',
        age: [30, 30, 30, 30, 31, 31, 33, 33, 33],
        person_id: []
    }
]
